<template>
    <div>
        <v-form id="productCategory" ref="productCategory" @submit.prevent="productCategory" autocomplete="nope">
            <Header :heading="getHeading()" >
            <v-switch
                hide-details
                class="mr-8"
                v-model="product_category.status"
                inset
                name="status"
                label="Status"
                color="success"
                :value="product_category.status"
            ></v-switch>
              ...
              <v-btn to="/product/category" color="info">
            ...
            <v-card class="full">
                 ...
                 <v-tab-item class="pa-5" :eager="true">
                   <v-text-field
                     v-model="product_category.title"
                     label="Nombre*"
                     name="name"
                     required
                     :rules="validateRules"
    ...
    beforeCreate() {
      ...
      * */
      ...
      if (this.$router.history.current.params && this.$router.history.current.params.id) {
        url = '/product/category/edit/' + this.$router.history.current.params.id;
      }
      ...
      */
      return this.$axiosx.get(url).then((res) => {
        ...
        if (this.product_category.status == 1) {
          this.product_category.status == true;
        } else {
          this.product_category.status == false;
        }
      });
    },
    data () {
    ...
    methods: {
      ...
      getData () {
        if (this.$route.params && this.$route.params.id) {
              var url = '/product/category/edit/' + this.$route.params.id;
      ...
      getHeading () {
          if (this.product_category && this.product_category.title) {
                return this.product_category.title;
          }
      ...
      productCategory () {
        ...
        if (this.$refs.productCategory.validate() == false) {
        ...
        var fd = new FormData(this.$refs.productCategory.$el);
      ...
      added() {
        ...
        if (this.$router.history.current.query.added == 'true') {
          this.$store.commit('snackbar', {
            status: 'success',
            text: 'La categoría de productos ha sido agregada.'
          });
        }
      }
    }
